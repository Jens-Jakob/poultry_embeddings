...
Use DINOv3 (self-supervised ViT) to embed images via HuggingFace Transformers. Compare embeddings via cosine similarity. Return ranked results.

**Phase 1 (current):** Global `[CLS]` token embeddings via DINOv3 ViT-B/16. No bounding boxes. Full-image similarity. Evaluate by eyeball — do the top results look like the reference?
